...
class RandomForestClassifierExample extends FunSuite with DataFrameSuiteBase {
  test("") {
    // 1. 加载数据
    ...
    val pipeline = new Pipeline()
      .setStages(Array(
        labelIndexerModel,// 这里是作为转换器
        featureIndexerModel,// 这里是作为转换器
        rf,// 这里是作为估测器
        labelConverter
      ))
    ...
    // 9. 进行预测
    val predictResults = model.transform(testData)
